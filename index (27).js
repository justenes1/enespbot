...
const commands = [
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('deliverables').setDescription('Format: (item1), (item2), (item3) OR .txt file URL (each line = 1 item)').setRequired(false)),
...
// ========================
function generateCommandJsonFiles() {
    ...
    const commandData = [
        { name: 'products', description: 'View all available products', options: [], permissions: 'everyone' },
        { name: 'orders', description: 'View your order history', options: [], permissions: 'everyone' },
        { name: 'help', description: 'Show all available commands', options: [], permissions: 'everyone' },
        { 
            name: 'addproduct', 
            description: 'Add a new product (Owner only)', 
            options: [
                { name: 'name', description: 'Product name', type: 'STRING', required: true },
                { name: 'usd_price', description: 'Price in USD', type: 'NUMBER', required: true },
                { name: 'description', description: 'Product description', type: 'STRING', required: false },
                { name: 'stock', description: 'Initial stock', type: 'INTEGER', required: false },
                { name: 'deliverables', description: 'Format: (item1), (item2), (item3) OR .txt file URL (each line = 1 item)', type: 'STRING', required: false }
            ],
            permissions: 'owner' 
        },
        { name: 'stock', description: 'View product stock (Owner only)', options: [], permissions: 'owner' },
        { name: 'setuptickets', description: 'Send the ticket panel to the configured channel (Owner only)', options: [], permissions: 'owner' },
        { 
            name: 'send', 
            description: 'Send an embed message to a channel (Owner only)', 
            options: [
                { name: 'channel', description: 'The channel to send the message to', type: 'CHANNEL', required: true },
                { name: 'message', description: 'The message to send', type: 'STRING', required: true },
                { name: 'image', description: 'Image URL (optional)', type: 'STRING', required: false }
            ],
            permissions: 'owner' 
        },
        { 
            ...
            options: [{ name: 'product', description: 'Product name to delete', type: 'STRING', required: true }],
            permissions: 'owner' 
        },
        { 
            name: 'editproduct', 
            description: 'Edit a product (Owner only)', 
            options: [{ name: 'product', description: 'Product name to edit', type: 'STRING', required: true }],
            permissions: 'owner' 
        },
        { 
            name: 'hold', 
            description: 'Put a product on hold or remove hold (Owner only)', 
            options: [{ name: 'product', description: 'Product name to hold/unhold', type: 'STRING', required: true }],
            permissions: 'owner' 
        }
    ];

    // Write each command to its own JSON file
    for (const cmd of commandData) {
        const filePath = path.join(commandsDir, `${cmd.name}.json`);
        fs.writeFileSync(filePath, JSON.stringify(cmd, null, 2));
    }

    console.log(`📝 Generated ${commandData.length} command JSON files in commands/`);
}
    ...
    if (client.editSessions.has(editKey)) {
        ...
        if (session.step === 2) {
            ...
            const embed = new EmbedBuilder()
                .setTitle('✏️ Edit Product')
                .setColor(0x00AAFF)
                .setDescription(`**Step 3/4: Would you like to change the product's description?**\n\nType a new description or type \`skip\`/\`next\` to keep it the same.`)
        ...
        if (session.step === 3) {
            ...
            // Parse current deliverables
            let currentDeliverables = [];
            try {
                currentDeliverables = JSON.parse(session.currentDeliverables || '[]');
            } catch (e) {
                currentDeliverables = [];
            }
            const deliverableText = currentDeliverables.length > 0 
                ? currentDeliverables.map((d, i) => `${i + 1}. ${d}`).join('\n') 
                : 'No deliverables';

            const embed = new EmbedBuilder()
                .setTitle('✏️ Edit Product')
                .setColor(0x00AAFF)
                .setDescription(`**Step 4/4: Would you like to change the product's deliverables?**\n\nFormat: \`(item1), (item2), (item3)\` OR paste a .txt file URL (each line = 1 item)\n\nType \`skip\`/\`next\` to keep current deliverables.`)
                .addFields({ name: 'Current Deliverables', value: deliverableText.substring(0, 1024) })
                .setTimestamp();
        ...
        if (session.step === 4) {
            if (!isSkip) {
                ...
                // Process deliverables - if .txt URL, fetch and parse each line; otherwise parse (item1), (item2) format
                async function processDeliverables(inputText) {
                    if (!inputText) return [];
                    
                    // Check if it's a .txt file URL
                    if (inputText.toLowerCase().endsWith('.txt') && (inputText.startsWith('http://') || inputText.startsWith('https://'))) {
                        try {
                            const response = await axios.get(inputText);
                            const lines = response.data.split('\n').map(line => line.trim()).filter(line => line.length > 0);
                            return lines;
                        } catch (error) {
                            console.error('Error fetching .txt file:', error.message);
                            return [];
                        }
                    }
                ...
                const deliverables = await processDeliverables(input);
                session.updates.deliverables = JSON.stringify(deliverables);
            }
            ...
            const changesText = Object.entries(session.updates)
                .map(([key, value]) => {
                    ...
                    if (key === 'deliverables') {
                        ...
                        return `**Deliverables:** ${items.length} item(s)`;
                    }
                    return `${key}: ${value}`;
                })
                .join('\n');
...
async function handleCommand(interaction, client) {
    ...
    if (commandName === 'addproduct') {
        ...
        const deliverablesInput = interaction.options.getString('deliverables') || '';

        // Process deliverables - if .txt URL, fetch and parse each line; otherwise parse (item1), (item2) format
        async function processDeliverables(input) {
            if (!input) return [];
            
            // Check if it's a .txt file URL
            if (input.toLowerCase().endsWith('.txt') && (input.startsWith('http://') || input.startsWith('https://'))) {
                try {
        ...
        const allDeliverables = await processDeliverables(deliverablesInput);
        const deliverablesJson = JSON.stringify(allDeliverables);

        // LTC price will be calculated or set to 0 (you can add conversion logic later)
        const ltcPrice = 0;

        const result = db.addProduct(name, description, ltcPrice, usdPrice, stock, null, guildId, interaction.user.id, deliverablesJson);
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Deliverables', value: `${allDeliverables.length} item(s)`, inline: true }
            )
            .setTimestamp();

        console.log(`📦 Product added: ${result.productId} - ${name} by ${interaction.user.id} with ${allDeliverables.length} deliverables`);
        await interaction.reply({ embeds: [embed], ephemeral: true });
        return;
    }
...
**/send** - Send embed message to a channel`
    ...
    if (commandName === 'editproduct') {
        ...
        // Parse current deliverables for display
        let currentDeliverables = [];
        try {
            currentDeliverables = JSON.parse(product.deliverables || '[]');
        } catch (e) {
            currentDeliverables = [];
        }
        ...
        client.editSessions.set(`edit_${interaction.user.id}_${interaction.channel.id}`, {
            ...
            currentDeliverables: product.deliverables || '[]',
            userId: interaction.user.id,
            channelId: interaction.channel.id,
            step: 1, // 1 = name, 2 = price, 3 = description, 4 = deliverables
            updates: {},
            createdAt: Date.now()
        });
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Current Deliverables', value: `${currentDeliverables.length} item(s)` }
            )
            .setTimestamp();
...
async function handleButton(interaction, client) {
    ...
    // Start purchase in ticket (automatic flow)
    ...
    // Manual purchase - ping owner
    if (customId.startsWith('purchase_manually_')) {
        const guildId = customId.replace('purchase_manually_', '');
        const embed = new EmbedBuilder()
            .setTitle('📝 Manual Purchase Selected')
            .setColor(0xFFAA00)
            .setDescription(`<@${interaction.user.id}> has chosen to purchase a product manually.\n\nPlease wait for the owner to assist you.`)
            .setTimestamp();

        await interaction.reply({ content: `<@${BOT_OWNER_ID}>`, embeds: [embed] });
        return;
    }

    // Automatic purchase - show product selection
    if (customId.startsWith('purchase_automatically_')) {
        const guildId = customId.replace('purchase_automatically_', '');
        ...
        const options = products.map((product, index) => ({
            label: `${product.name}`,
            description: `$${product.usd_price || 'N/A'} | Stock: ${product.stock}`,
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(0x00AAFF)
            .setDescription('Choose the product you want to purchase:');

        let productList = '';
        products.forEach((product, index) => {
            productList += `• **${product.name}** - $${product.usd_price || 'N/A'}\n`;
        });
        embed.addFields({ name: 'Available Products', value: productList || 'No products' });
...
async function createPurchaseTicket(interaction, client) {
    ...
    // Show purchase method selection
    const purchaseMethodEmbed = new EmbedBuilder()
        .setTitle('🛒 How would you like to purchase?')
        .setColor(0x00AAFF)
        .setDescription('Please select your preferred purchase method below:');

    const methodRow = new ActionRowBuilder()
        .addComponents(
            new ButtonBuilder()
                .setCustomId(`purchase_manually_${guildId}`)
                .setLabel('📝 Manually')
                .setStyle(ButtonStyle.Secondary),
            new ButtonBuilder()
                .setCustomId(`purchase_automatically_${guildId}`)
                .setLabel('⚡ Automatically')
                .setStyle(ButtonStyle.Success)
        );

    await ticketChannel.send({ embeds: [purchaseMethodEmbed], components: [methodRow] });
